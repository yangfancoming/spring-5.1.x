...
class MapperFactoryBeanTest extends AbstractMyBatisSpringTest {
  ...
  void testNonSpringTxMgrWithTx() throws Exception {
    ...
    try {
      ...
      assertThat(e.getMessage()).isEqualTo("SqlSessionFactory must be using a SpringManagedTransactionFactory in order to use Spring transaction synchronization");
    } finally {
