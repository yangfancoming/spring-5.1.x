...
/**
 * Test for the MapperScannerRegistrar. This test works fine with Spring 3.1 and 3.2 but with 3.1 the registrar is called twice.
 */
class NamespaceTest {
  ...
  void testScanWithMapperFactoryBeanClass() {
    ...
    applicationContext = new ClassPathXmlApplicationContext(new String[] { "org/mybatis/spring/config/mapper-factory-bean-class.xml" }, setupSqlSessionTemplate());
    startContext();
    // all interfaces with methods should be loaded
    applicationContext.getBean("mapperInterface");
    applicationContext.getBean("mapperSubinterface");
  ...
  @Test
  void testLazy() {
    applicationContext = new ClassPathXmlApplicationContext(new String[] { "org/mybatis/spring/config/lazy.xml" }, setupSqlSessionTemplate());
    startContext();
    SqlSessionFactory sqlSessionFactory = applicationContext.getBean(SqlSessionFactory.class);
