...
class MyBatisSpringTest extends AbstractMyBatisSpringTest {
  ...
  void testNonSpringTxFactoryWithTx() throws Exception {
    ...
    try {
      ...
      assertThrows(TransientDataAccessResourceException.class,() -> session = SqlSessionUtils.getSqlSession(sqlSessionFactory));
  ...
  void testChangeExecutorTypeInTx() throws Exception {
    ...
    try {
      status = txManager.getTransaction(new DefaultTransactionDefinition());
      session = SqlSessionUtils.getSqlSession(sqlSessionFactory);
      assertThrows(TransientDataAccessResourceException.class,() -> session = SqlSessionUtils.getSqlSession(sqlSessionFactory, ExecutorType.BATCH, exceptionTranslator));
