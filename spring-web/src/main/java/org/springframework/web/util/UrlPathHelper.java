...
/**
 * Helper class for URL path matching. Provides support for URL paths in {@code RequestDispatcher} includes and support for consistent URL decoding.
 * Used by {@link org.springframework.web.servlet.handler.AbstractUrlHandlerMapping}
 * and {@link org.springframework.web.servlet.support.RequestContext} for path matching and/or URI determination.
 * @since 14.01.2004
...
public class UrlPathHelper {
	 ...
	 * Preferable over the standard Servlet 2.4 forward attribute on WebSphere, simply because we need the very first URI in the request forwarding chain.
	 */
	 ...
	 * web application context, i.e. within {@link javax.servlet.ServletContext#getContextPath()}.
	 * If set to {@literal false} the path within the current servlet mapping
	 * is used instead if applicable (i.e. in the case of a prefix based Servlet mapping such as "/myServlet/*").
	 ...
	 * which are returned <i>undecoded</i> by the Servlet API, in contrast to the servlet path.
	 ...
	 * <strong>Note:</strong> Be aware the servlet path will not match when compared to encoded paths.
	 * Therefore use of {@code urlDecode=false} is not compatible with a prefix-based Servlet mapping and likewise implies also setting {@code alwaysUseFullPath=true}.
	 * @see #getServletPath
	 * @see #getContextPath
	 * @see #getRequestUri
	 ...
	 * This also allows for generically overriding the character encoding in a filter that invokes the {@code ServletRequest.setCharacterEncoding} method.
	...
	public String getPathWithinServletMapping(HttpServletRequest request) {
		...
		if (path != null) {
			...
			if (!this.urlDecode) {
				...
				if (path != null) return pathWithinApp;
			}
...
//	public String getRemainingPath(String requestUri, String mapping, boolean ignoreCase) {
	 ...
	 * As the value returned by {@code request.getContextPath()} is <i>not</i> decoded by the servlet container, this method will decode it.
	 * @param request current HTTP request
	...
	protected String determineEncoding(HttpServletRequest request) {
		...
		if (enc == null) enc = getDefaultEncoding();
		return enc;
	}
	...
	private boolean shouldRemoveTrailingServletPathSlash(HttpServletRequest request) {
		if (request.getAttribute(WEBSPHERE_URI_ATTRIBUTE) == null) {
			// Regular servlet container: behaves as expected in any case, so the trailing slash is the result of a "/" url-pattern mapping.
		...
		if (flagToUse == null) {
			...
			try {
				...
				if (logger.isDebugEnabled()) logger.debug("Could not introspect WebSphere web container properties: " + ex);
			}
			flagToUse = flag;
