 ...
 * and then off each resource the same PathMatcher resolution strategy described above is used for the wildcard subpath.
 * <b>Other notes:</b>
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			if (getPathMatcher().isPattern(locationPattern.substring(CLASSPATH_ALL_URL_PREFIX.length()))) {
				...
				// all class path resources with the given name   传入参数 example/scannable/
	 ...
	 * @param location the absolute path within the classpath    eg: example/scannable/
	...
	protected Resource[] findAllClassPathResources(String location) throws IOException {
		...
		if (logger.isTraceEnabled()) logger.trace("Resolved classpath location [" + location + "] to resources " + result);
	...
	protected void addAllClassLoaderJarRoots(@Nullable ClassLoader classLoader, Set<Resource> result) {
			...
			try {
				for (URL url : ((URLClassLoader) classLoader).getURLs()) {
					try {
						...
						if (logger.isDebugEnabled()) logger.debug("Cannot search for matching files underneath [" + url + "] because it cannot be converted to a valid 'jar:' URL: " + ex.getMessage());
					}
				}
			}catch (Exception ex) {
		...
		if (classLoader != null) {
			try {
				...
				if (logger.isDebugEnabled()) logger.debug("Cannot introspect jar files in parent ClassLoader since [" + classLoader + "] does not support 'getParent()': " + ex);
			}
		}
	}
		...
		try {
			...
			for (String path : StringUtils.delimitedListToStringArray(
				...
				try {
					...
					if (logger.isDebugEnabled()) logger.debug("Cannot search for matching files underneath [" + path + "] because it cannot be converted to a valid 'jar:' URL: " + ex.getMessage());
				}
			}
		}catch (Exception ex) {
			if (logger.isDebugEnabled()) logger.debug("Failed to evaluate 'java.class.path' manifest entries: " + ex);
		}
	}
	 ...
	 * Ant-style PathMatcher. Supports resources in jar files and zip files and in the file system.
	 * @param locationPattern the location pattern to match   classpath*:example/scannable/**'/*.class
	 * @return the result as Resource array
	 * @throws IOException in case of I/O errors
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		// locationPattern == classpath*:example/scannable/**/*.class
		String rootDirPath = determineRootDir(locationPattern); //rootDirPath == classpath*:example/scannable/
		String subPattern = locationPattern.substring(rootDirPath.length());//subPattern == **/*.class
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			if (equinoxResolveMethod != null && rootDirUrl.getProtocol().startsWith("bundle")) {
				...
				if (resolvedUrl != null) rootDirUrl = resolvedUrl;
		...
		if (logger.isTraceEnabled()) logger.trace("Resolved location pattern [" + locationPattern + "] to resources " + result);
		return result.toArray(new Resource[0]);
	}
	...
	/**
	 * Return whether the given resource handle indicates a jar resource that the {@code doFindPathMatchingJarResources} method can handle.
	...
	protected Set<Resource> doFindPathMatchingJarResources(Resource rootDirResource, URL rootDirURL, String subPattern) throws IOException {
		...
		if (con instanceof JarURLConnection) {
			...
			try {
				...
				if (logger.isDebugEnabled()) logger.debug("Skipping invalid jar classpath entry [" + urlFile + "]");
	...
	protected Set<Resource> doFindPathMatchingFileResources(Resource rootDirResource, String subPattern) 	throws IOException {
		...
		try {
			...
			if (logger.isInfoEnabled()) logger.info("Failed to resolve " + rootDirResource + " in the file system: " + ex);
			return Collections.emptySet();
		}
	...
	protected Set<Resource> doFindMatchingFileSystemResources(File rootDir, String subPattern) throws IOException {
		if (logger.isTraceEnabled()) logger.trace("Looking for matching resources in directory tree [" + rootDir.getPath() + "]");
	...
	protected Set<File> retrieveMatchingFiles(File rootDir, String pattern) throws IOException {
		if (!rootDir.exists()) {
			...
			if (logger.isDebugEnabled()) logger.debug("Skipping [" + rootDir.getAbsolutePath() + "] because it does not exist");
			return Collections.emptySet();
		}
		if (!rootDir.isDirectory()) {
			// Complain louder if it exists but is no directory.
			if (logger.isInfoEnabled()) logger.info("Skipping [" + rootDir.getAbsolutePath() + "] because it does not denote a directory");
			return Collections.emptySet();
		}
		if (!rootDir.canRead()) {
	...
	protected File[] listDirectory(File dir) {
		...
		if (files == null) {
			if (logger.isInfoEnabled()) logger.info("Could not retrieve contents of directory [" + dir.getAbsolutePath() + "]");
			return new File[0];
		}
