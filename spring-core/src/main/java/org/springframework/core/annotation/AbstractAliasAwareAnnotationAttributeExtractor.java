...
abstract class AbstractAliasAwareAnnotationAttributeExtractor<S> implements AnnotationAttributeExtractor<S> {
			...
			for (String aliasName : aliasNames) {
				...
				if (!ObjectUtils.nullSafeEquals(attributeValue, aliasValue) &&
					...
					throw new AnnotationConfigurationException(String.format(
							"In annotation [%s] declared on %s and synthesized from [%s], attribute '%s' and its alias '%s' are present with values of [%s] and [%s], but only one is permitted.",
	...
	/**
	 * Get the raw, unmodified attribute value from the underlying {@linkplain #getSource source} that corresponds to the supplied  attribute method.
	 */
	@Nullable
	protected abstract Object getRawAttributeValue(Method attributeMethod);

	/**
	 * Get the raw, unmodified attribute value from the underlying  {@linkplain #getSource source} that corresponds to the supplied attribute name.
	 */
	@Nullable
