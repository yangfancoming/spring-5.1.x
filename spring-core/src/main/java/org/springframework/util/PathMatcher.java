 ...
 * {@link org.springframework.web.servlet.handler.AbstractUrlHandlerMapping},and {@link org.springframework.web.servlet.mvc.WebContentInterceptor}.
...
public interface PathMatcher {
	 ...
	 * This method is supposed to find out which part of the path is matched dynamically through an actual pattern,
	 * that is, it strips off a statically  defined leading path from the given full path, returning only the actually  pattern-matched part of the path.
	 * For example: For "myroot/*.html" as pattern and "myroot/myfile.html"  as full path, this method should return "myfile.html".
	 * The detailed determination rules are specified to this PathMatcher's matching strategy.
	 * A simple implementation may return the given full path as-is in case of an actual pattern,
	 ...
	 * The full algorithm used depends on the underlying implementation,but generally, the returned {@code Comparator} will
	 * {@linkplain java.util.List#sort(java.util.Comparator) sort} a list so that more specific patterns come before generic patterns.
