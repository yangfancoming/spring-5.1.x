...
public interface WebMvcConfigurer {
	 ...
	 * <strong>Note</strong> that interceptors registered here only apply to controllers and not to resource handler requests.
	 * To intercept requests for static resources either declare a {@link org.springframework.web.servlet.handler.MappedInterceptor MappedInterceptor}
	 ...
	 * <strong>Note</strong> that adding converters to the list, turns off default converter registration.
	 ...
	 * This may be useful for example to allow default converters to be registered and then insert a custom converter through this method.
	 ...
	 * Or if any exception resolvers are added to the list, then the application effectively takes over and must provide, fully initialized, exception resolvers.
	 * Alternatively you can use {@link #extendHandlerExceptionResolvers(List)} which allows you to extend
	 ...
	 * The default implementation, assuming JSR-303 is on the classpath, is: {@link org.springframework.validation.beanvalidation.OptionalValidatorFactoryBean}.
	...
	/**
	 * Provide a custom {@link MessageCodesResolver} for building message codes from data binding and validation error codes.
	 * Leave the return value as {@code null} to keep the default.
	 */
	@Nullable
