...
public class MarshallingView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if (toBeMarshalled == null) throw new IllegalStateException("Unable to locate object to be marshalled in model: " + model);
	 ...
	 * {@linkplain #setModelKey(String) model key}, if any, before attempting to locate an object of {@linkplain Marshaller#supports(Class) supported type}.
	...
	protected Object locateToBeMarshalled(Map<String, Object> model) throws IllegalStateException {
		if (this.modelKey != null) {
			...
			if (value == null) throw new IllegalStateException("Model contains no object with key [" + this.modelKey + "]");
		...
		for (Map.Entry<String, Object> entry : model.entrySet()) {
			...
			if (value != null && (model.size() == 1 || !(value instanceof BindingResult)) && isEligibleForMarshalling(entry.getKey(), value)) {
				return value;
			}
		}
		return null;
