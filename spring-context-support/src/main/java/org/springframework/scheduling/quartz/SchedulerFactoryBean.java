 ...
 *
 * xmlBeanDefinitionReaderAllows registration of JobDetails, Calendars and Triggers, automatically
 ...
 *
 * xmlBeanDefinitionReaderFor dynamic registration of jobs at runtime, use a bean reference to
 ...
 *
 * xmlBeanDefinitionReaderNote that Quartz instantiates a new Job for each execution, in
 ...
 *
 * xmlBeanDefinitionReaderWhen using persistent jobs, it is strongly recommended to perform all
 ...
 *
 * xmlBeanDefinitionReaderThe preferred way to achieve transactional execution is to demarcate
...
public class SchedulerFactoryBean extends SchedulerAccessor implements FactoryBean<Scheduler>,BeanNameAware, ApplicationContextAware, InitializingBean, DisposableBean, SmartLifecycle {
	 ...
	 * xmlBeanDefinitionReaderThis instance will be set before initialization of the corresponding Scheduler,
	 ...
	 * xmlBeanDefinitionReaderDefault is an internal {@link StdSchedulerFactory} instance.
	 * If this method is called, it overrides any class specified through {@link #setSchedulerFactoryClass} as well as any settings specified through {@link #setConfigLocation},
	 * {@link #setQuartzProperties}, {@link #setTaskExecutor} or {@link #setDataSource}.
	 * xmlBeanDefinitionReader<b>NOTE:</b> With an externally provided {@code SchedulerFactory} instance,
	 * local settings such as {@link #setConfigLocation} or {@link #setQuartzProperties}  will be ignored here in {@code SchedulerFactoryBean},
	 ...
	 * xmlBeanDefinitionReaderDefault is the {@link StdSchedulerFactory} class, reading in the standard  {@code quartz.properties} from {@code quartz.jar}.
	 * For applying custom Quartz properties, specify {@link #setConfigLocation "configLocation"} and/or
	 * {@link #setQuartzProperties "quartzProperties"} etc on this local {@code SchedulerFactoryBean} instance.
	 * @see org.quartz.impl.StdSchedulerFactory
	...
	/**
	 * Set the name of the Scheduler to create via the SchedulerFactory, as an alternative to the {@code org.quartz.scheduler.instanceName} property.
	 * xmlBeanDefinitionReaderIf not specified, the name will be taken from Quartz properties ({@code org.quartz.scheduler.instanceName}), or from the declared
	 ...
	 * xmlBeanDefinitionReaderNote: Can be omitted when all necessary properties are specified
	 ...
	 * xmlBeanDefinitionReaderCan be used to override values in a Quartz properties config file,
	 ...
	 * xmlBeanDefinitionReaderCan be used to assign a local JDK ThreadPoolExecutor or a CommonJ
	 * WorkManager as Quartz backend, to avoid Quartz's manual thread creation.
	 * xmlBeanDefinitionReaderBy default, a Quartz SimpleThreadPool will be used, configured through
	 ...
	 * xmlBeanDefinitionReaderNote: If this is set, the Quartz settings should not define
	 * a job store "dataSource" to avoid meaningless double configuration.
	 * xmlBeanDefinitionReaderA Spring-specific subclass of Quartz' JobStoreCMT will be used.
	 * It is therefore strongly recommended to perform all operations on
	 * the Scheduler within Spring-managed (or plain JTA) transactions.
	 * Else, database locking will not properly work and might even break
	 * (e.g. if trying to obtain a lock on Oracle without a transaction).
	 * xmlBeanDefinitionReaderSupports both transactional and non-transactional DataSource access.
	 ...
	 * xmlBeanDefinitionReaderThis is only necessary if the default DataSource is an XA DataSource that will
	 ...
	 * xmlBeanDefinitionReaderThis is not relevant with a local DataSource instance and Spring transactions.
	 ...
	 * xmlBeanDefinitionReaderNote: When using persistent Jobs whose JobDetail will be kept in the
	 ...
	 * xmlBeanDefinitionReaderIn case of a QuartzJobBean, the reference will be applied to the Job
	 ...
	 * xmlBeanDefinitionReaderNote that BeanFactory callback interfaces like ApplicationContextAware
	 ...
	 * xmlBeanDefinitionReaderDefault is Spring's {@link AdaptableJobFactory}, which supports
	 ...
	 * xmlBeanDefinitionReaderSpecify an instance of Spring's {@link SpringBeanJobFactory} here
	 ...
	 * xmlBeanDefinitionReaderDefault is "true"; set this to "false" to allow for manual startup.
	 */
	 ...
	 * xmlBeanDefinitionReaderSetting this to 10 or 20 seconds makes sense if no jobs
	 ...
	 * xmlBeanDefinitionReaderSwitch this flag to "true" in order to expose the Scheduler globally.
	 ...
	 * xmlBeanDefinitionReaderDefault is "false". Switch this to "true" if you prefer
	...
	private void initSchedulerFactory(StdSchedulerFactory schedulerFactory) throws SchedulerException, IOException {
		...
		if (this.configLocation != null) {
			if (logger.isDebugEnabled()) logger.debug("Loading Quartz config from [" + this.configLocation + "]");
	 ...
	 * xmlBeanDefinitionReaderThe default implementation invokes SchedulerFactory's {@code getScheduler} method. Can be overridden for custom Scheduler creation.
	...
	protected void startScheduler(final Scheduler scheduler, final int startupDelay) throws SchedulerException {
		if (startupDelay <= 0) {
			...
			if (logger.isInfoEnabled()) logger.info("Will start Quartz Scheduler [" + scheduler.getSchedulerName() + "] in " + startupDelay + " seconds");
			...
			Thread schedulerThread = new Thread() {
				...
				public void run() {
					...
					if (logger.isInfoEnabled()) logger.info("Starting Quartz Scheduler now, after delay of " + startupDelay + " seconds");
					try {
						scheduler.start();
