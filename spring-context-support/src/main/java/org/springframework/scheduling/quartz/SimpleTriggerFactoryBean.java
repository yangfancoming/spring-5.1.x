 ...
 *
 * xmlBeanDefinitionReader{@code SimpleTrigger(Impl)} itself is already a JavaBean but lacks sensible defaults.
 ...
 *
 * xmlBeanDefinitionReaderThis class will also register the trigger with the job name and group of
...
public class SimpleTriggerFactoryBean implements FactoryBean<SimpleTrigger>, BeanNameAware, InitializingBean {
	 ...
	 * xmlBeanDefinitionReaderThese objects will be available to this Trigger only,
	 ...
	 * xmlBeanDefinitionReaderNote that a dynamically computed {@link #setStartDelay} specification
	 ...
	 * xmlBeanDefinitionReaderThe start delay is added to the current system time (when the bean starts)
	 ...
	 * xmlBeanDefinitionReaderDefault is to repeat indefinitely.
	 */
