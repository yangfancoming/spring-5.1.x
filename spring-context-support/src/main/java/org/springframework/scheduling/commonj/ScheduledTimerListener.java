 ...
 *
 * xmlBeanDefinitionReaderThe CommonJ TimerManager does not offer more sophisticated scheduling
 * options such as cron expressions. Consider using Quartz for such
 * advanced needs.
 *
 * xmlBeanDefinitionReaderNote that the TimerManager uses a TimerListener instance that is
...
public class ScheduledTimerListener {
	 ...
	 * xmlBeanDefinitionReaderIf the "firstTime" property is specified, this property will be ignored.
	 ...
	 * xmlBeanDefinitionReaderDefault is -1, leading to one-time execution. In case of zero or a
	 * positive value, the task will be executed repeatedly, with the given
	 * interval in-between executions.
	 * xmlBeanDefinitionReaderNote that the semantics of the period value vary between fixed-rate
	 * and fixed-delay execution.
	 * xmlBeanDefinitionReader<b>Note:</b> A period of 0 (for example as fixed delay) <i>is</i>
	 ...
	 * xmlBeanDefinitionReaderSee TimerManager javadoc for details on those execution modes.
	 * @see commonj.timers.TimerManager#schedule(commonj.timers.TimerListener, long, long)
