...
public abstract class AopNamespaceUtils {
	...
	public static void registerAspectJAnnotationAutoProxyCreatorIfNecessary(ParserContext parserContext, Element sourceElement) {
		...
		useClassProxyingIfNecessary(parserContext.getRegistry(), sourceElement);
		// 注册组件并通知，便于监听器进一步处理，这里可以不用看  // 将注册的BeanDefinition封装到BeanComponentDefinition中
		registerComponentIfNecessary(beanDefinition, parserContext);
	}
	...
	private static void useClassProxyingIfNecessary(BeanDefinitionRegistry registry, @Nullable Element sourceElement) {
		if (sourceElement != null) {
			...
			if (proxyTargetClass) {
				// 将解析得到的proxy-target-class属性值设置到上面生成的AnnotationAwareAspectJAutoProxyCreator的BeanDefinition的proxyTargetClass属性中
				AopConfigUtils.forceAutoProxyCreatorToUseClassProxying(registry);
			}
			// 解析标签中的expose-proxy属性值
			boolean exposeProxy = Boolean.parseBoolean(sourceElement.getAttribute(EXPOSE_PROXY_ATTRIBUTE));
			if (exposeProxy) {
				// 将解析得到的expose-proxy属性值设置到 AnnotationAwareAspectJAutoProxyCreator的exposeProxy属性中
	...
	private static void registerComponentIfNecessary(@Nullable BeanDefinition beanDefinition, ParserContext parserContext) {
		// 如果生成的AnnotationAwareAspectJAutoProxyCreator的BeanDefinition成功，则将其封装到BeanComponentDefinition中，并且将其添加到ParserContext中
