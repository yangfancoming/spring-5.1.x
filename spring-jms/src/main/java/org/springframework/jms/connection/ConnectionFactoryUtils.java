 ...
 *
 * xmlBeanDefinitionReaderMainly for internal use within the framework. Used by
...
public abstract class ConnectionFactoryUtils {
	 ...
	 * xmlBeanDefinitionReaderChecks {@link SmartConnectionFactory#shouldStop}, if available.
	 ...
	 * xmlBeanDefinitionReaderMainly intended for use with the JMS 1.0.2 API.
	 * @param cf the ConnectionFactory to obtain a Session for
	 * @param existingCon the existing JMS Connection to obtain a Session for
	 * (may be {@code null})
	 * @param synchedLocalTransactionAllowed whether to allow for a local JMS transaction
	...
	/**
	 * Obtain a JMS Session that is synchronized with the current transaction, if any.
	 * xmlBeanDefinitionReaderThis {@code doGetTransactionalSession} variant always starts the underlying
