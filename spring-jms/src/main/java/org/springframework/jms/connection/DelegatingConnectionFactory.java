 ...
 *
 * xmlBeanDefinitionReaderThis class allows for being subclassed, with subclasses overriding only
 * those methods (such as {@link #createConnection()}) that should not simply
 * delegate to the target ConnectionFactory.
 *
 * xmlBeanDefinitionReaderCan also be defined as-is, wrapping a specific target ConnectionFactory,
...
public class DelegatingConnectionFactory
	 ...
	 * released connections to the pool, not stopping them while they sit in the pool.
	 * xmlBeanDefinitionReaderDefault is "false", simply closing Connections.
	 * @see ConnectionFactoryUtils#releaseConnection
	 */
