 ...
 *
 * xmlBeanDefinitionReaderUsually holds a single JMS {@link Connection} that all listeners are supposed
 ...
 * subclasses.
 *
 * xmlBeanDefinitionReader<b>NOTE:</b> The default behavior of this message listener container is to
 ...
 *
 * xmlBeanDefinitionReaderThe listener container offers the following message acknowledgment options:
 * <ul>
 ...
 * </ul>
 *
 * xmlBeanDefinitionReaderThere are two solutions to the duplicate message processing problem:
 * <ul>
 ...
 *
 * xmlBeanDefinitionReader<b>Recommendations:</b>
 * <ul>
 ...
 * </ul>
 *
 * xmlBeanDefinitionReaderNote that the "sessionTransacted" flag is strongly recommended over
...
public abstract class AbstractMessageListenerContainer extends AbstractJmsListeningContainer
	 ...
	 * xmlBeanDefinitionReaderAlternatively, specify a "destinationName", to be dynamically
	 * resolved via the {@link org.springframework.jms.support.destination.DestinationResolver}.
	 * xmlBeanDefinitionReaderNote: The destination may be replaced at runtime, with the listener
	 ...
	 * xmlBeanDefinitionReaderThe specified name will be dynamically resolved via the configured
	 * {@link #setDestinationResolver destination resolver}.
	 * xmlBeanDefinitionReaderAlternatively, specify a JMS {@link Destination} object as "destination".
	 * xmlBeanDefinitionReaderNote: The destination may be replaced at runtime, with the listener
	 * container picking up the new destination immediately (works e.g. with
	 * DefaultMessageListenerContainer, as long as the cache level is less than
	 * CACHE_CONSUMER). However, this is considered advanced usage; use it with care!
	 ...
	 * xmlBeanDefinitionReaderNote: The message selector may be replaced at runtime, with the listener
	 ...
	 * xmlBeanDefinitionReaderNote: The message listener may be replaced at runtime, with the listener
	 ...
	 * xmlBeanDefinitionReaderBy default, only a standard JMS {@link MessageListener} object or a
	 ...
	 * xmlBeanDefinitionReaderDefault is "false". Set this to "true" to register a durable subscription,
	 ...
	 * xmlBeanDefinitionReaderOnly makes sense when listening to a topic (pub-sub domain),
	 ...
	 * to be used can be specified through the "subscriptionName" property.
	 * xmlBeanDefinitionReaderDefault is "false". Set this to "true" to register a shared subscription,
	 * typically in combination with a "subscriptionName" value (unless
	 * your message listener class name is good enough as subscription name).
	 ...
	 * xmlBeanDefinitionReaderOnly makes sense when listening to a topic (pub-sub domain),
	 * therefore this method switches the "pubSubDomain" flag as well.
	 * xmlBeanDefinitionReader<b>Requires a JMS 2.0 compatible message broker.</b>
	 * @since 4.1
	 * @see #setSubscriptionName
	 ...
	 * xmlBeanDefinitionReaderThe subscription name needs to be unique within this client's
	 * JMS client id. Default is the class name of the specified message listener.
	 * xmlBeanDefinitionReaderNote: Only 1 concurrent consumer (which is the default of this
	 ...
	 * xmlBeanDefinitionReaderThe durable subscription name needs to be unique within this client's
	 * JMS client id. Default is the class name of the specified message listener.
	 * xmlBeanDefinitionReaderNote: Only 1 concurrent consumer (which is the default of this
	 ...
	 * xmlBeanDefinitionReaderThis setting primarily indicates what type of destination to resolve if dynamic
	 ...
	 * xmlBeanDefinitionReaderBy default, there will be <b>no</b> ErrorHandler so that error-level
	 ...
	 * xmlBeanDefinitionReaderDefault is "true", reusing the listener's {@link Session}.
	 ...
	 * xmlBeanDefinitionReaderNote that Sessions managed by an external transaction manager will
	 ...
	 * xmlBeanDefinitionReaderDefault is "false", rejecting such messages through aborting the
	 ...
	 * xmlBeanDefinitionReader<b>NOTE:</b> Aborting receive attempts for such incoming messages
	 ...
	 * xmlBeanDefinitionReaderDefault implementation performs a plain invocation of the
	 ...
	 * xmlBeanDefinitionReaderNote: The Session's own transacted flag will already have been checked
	 ...
	 * xmlBeanDefinitionReaderThis implementation uses JMS 1.1 API.
