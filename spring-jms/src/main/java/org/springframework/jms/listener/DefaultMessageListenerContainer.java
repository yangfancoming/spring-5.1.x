 ...
 *
 * xmlBeanDefinitionReaderThis is a simple but nevertheless powerful form of message listener container.
 ...
 *
 * xmlBeanDefinitionReaderActual {@code MessageListener} execution happens in asynchronous work units which are
 ...
 *
 * xmlBeanDefinitionReaderMessage reception and listener execution can automatically be wrapped
 ...
 *
 * xmlBeanDefinitionReaderDynamic scaling of the number of concurrent invokers can be activated
 ...
 * the same node.
 *
 * xmlBeanDefinitionReader<b>Note: Don't use Spring's {@link org.springframework.jms.connection.CachingConnectionFactory}
 ...
 *
 * xmlBeanDefinitionReader<b>It is strongly recommended to either set {@link #setSessionTransacted
...
public class DefaultMessageListenerContainer extends AbstractPollingMessageListenerContainer {
	 ...
	 * xmlBeanDefinitionReaderDefault is a {@link org.springframework.core.task.SimpleAsyncTaskExecutor},
	 * starting up a number of new threads, according to the specified number
	 * of concurrent consumers.
	 * xmlBeanDefinitionReaderSpecify an alternative {@code TaskExecutor} for integration with an existing
	 ...
	 * attempt to recover.
	 * xmlBeanDefinitionReaderThe {@link #setRecoveryInterval(long) recovery interval} is ignored
	 ...
	 * xmlBeanDefinitionReaderFor more recovery options, consider specifying a {@link BackOff}
	 * instance instead.
	...
	/**
	 * Specify the level of caching that this listener container is allowed to apply.
	 * xmlBeanDefinitionReaderDefault is {@link #CACHE_NONE} if an external transaction manager has been specified
	 ...
	 * xmlBeanDefinitionReaderSome Java EE servers only register their JMS resources with an ongoing XA
	 ...
	 * xmlBeanDefinitionReaderThis listener container will always hold on to the minimum number of consumers
	 ...
	 * xmlBeanDefinitionReaderSpecifying a higher value for this setting will increase the standard
	 ...
	 * xmlBeanDefinitionReaderRaising the number of concurrent consumers is recommendable in order
	 * to scale the consumption of messages coming in from a queue. However,
	 * note that any ordering guarantees are lost once multiple consumers are
	 * registered. In general, stick with 1 consumer for low-volume queues.
	 * xmlBeanDefinitionReader<b>Do not raise the number of concurrent consumers for a topic,
	 ...
	 * xmlBeanDefinitionReader<b>This setting can be modified at runtime, for example through JMX.</b>
	 ...
	 * xmlBeanDefinitionReaderThis returns the currently configured "concurrentConsumers" value;
	 ...
	 * xmlBeanDefinitionReaderIf this setting is higher than "concurrentConsumers", the listener container
	 ...
	 * xmlBeanDefinitionReaderThis returns the currently configured "maxConcurrentConsumers" value;
	 * the number of currently scheduled/active consumers might differ.
	 * @see #getScheduledConsumerCount()
	 * @see #getActiveConsumerCount()
	 */
	 ...
	 * xmlBeanDefinitionReaderDefault is unlimited (-1) in case of a standard TaskExecutor,
	 * reusing the original invoker threads until shutdown (at the
	 * expense of limited dynamic scheduling).
	 * xmlBeanDefinitionReaderIn case of a SchedulingTaskExecutor indicating a preference for
	 ...
	 * xmlBeanDefinitionReaderLong-lived tasks avoid frequent thread context switches through
	 ...
	 * at any given time.
	 * xmlBeanDefinitionReaderThis limit is used by the {@link #scheduleNewInvokerIfAppropriate} method
	 ...
	 * xmlBeanDefinitionReaderThe default is 1, only scheduling a new invoker (which is likely to
	 ...
	 * xmlBeanDefinitionReaderThe default is 1, closing idle resources early once a task didn't
	 ...
	 * xmlBeanDefinitionReaderWithin each task execution, a number of message reception attempts
	 ...
	 * xmlBeanDefinitionReaderRaise this limit if you encounter too frequent scaling up and down.
	 ...
	 * xmlBeanDefinitionReaderNote: Further {@code stop(runnable)} calls (before processing
	 * has actually stopped) will override the specified callback. Only the
	 * latest specified callback will be invoked.
	 * xmlBeanDefinitionReaderIf a subsequent {@link #start()} call restarts the listener container
	 ...
	 * xmlBeanDefinitionReaderThis number will always be between "concurrentConsumers" and
	 ...
	 * xmlBeanDefinitionReaderThis method may be polled after a {@link #start()} call, until asynchronous
	 ...
	 * xmlBeanDefinitionReaderNote that a listener container is not bound to having a fixed registration in
	 ...
	 * xmlBeanDefinitionReaderThe default implementation builds a {@link org.springframework.core.task.SimpleAsyncTaskExecutor}
	 ...
	 * xmlBeanDefinitionReaderCalled once a message has been received, in order to scale up while
	 ...
	 * xmlBeanDefinitionReaderThe default implementation logs the exception at warn level
	 ...
	 * xmlBeanDefinitionReaderThe default implementation delegates to DefaultMessageListenerContainer's
	 ...
	 * xmlBeanDefinitionReaderThe default implementation retries until it successfully established a
	 ...
	 * xmlBeanDefinitionReaderCalled after listener setup failure, assuming that a cached Destination
	 * object might have become invalid (a typical case on WebLogic JMS).
	 * xmlBeanDefinitionReaderThe default implementation removes the destination from a
	 ...
	 * xmlBeanDefinitionReaderReturn {@code true} if the back-off period has been applied and a new
	 ...
	 * xmlBeanDefinitionReaderMay be used to detect recovery phases but also the end of a recovery phase,
