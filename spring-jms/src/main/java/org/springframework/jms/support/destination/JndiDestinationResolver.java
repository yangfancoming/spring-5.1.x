 ...
 *
 * xmlBeanDefinitionReaderAllows for customizing the JNDI environment if necessary, for example
 * specifying appropriate JNDI environment properties.
 *
 * xmlBeanDefinitionReaderDynamic queues and topics get cached by destination name. As a consequence,
 ...
 *
 * xmlBeanDefinitionReaderNote that the fallback to resolution of dynamic destinations
...
public class JndiDestinationResolver extends JndiLocatorSupport implements CachingDestinationResolver {
	 ...
	 * xmlBeanDefinitionReaderThis flag can be turned off to re-lookup a destination for each operation,
	 * which allows for hot restarting of destinations. This is mainly useful
	 * during development.
	 * xmlBeanDefinitionReaderNote that dynamic queues and topics get cached by destination name.
	 ...
	 * xmlBeanDefinitionReaderTurn this flag on to enable transparent fallback to dynamic destinations.
	 ...
	 * destinations.
	 * xmlBeanDefinitionReaderThe default is Spring's standard {@link DynamicDestinationResolver}.
	 * @see #setFallbackToDynamicDestination
