...
/**
 *
 * Extends the {@link ServletResponse} interface to provide HTTP-specific functionality in sending a response.
 ...
 * and passes it as an argument to the servlet's service methods  (<code>doGet</code>, <code>doPost</code>, etc).
...
public interface HttpServletResponse extends ServletResponse {
     ...
     * no error (for example, for the SC_OK or SC_MOVED_TEMPORARILY status codes).
     *
     ...
     * message parameter. To set a status code   use <code>setStatus(int)</code>, to send an error with a description
     ...
     * {@link #addDateHeader}, {@link #setIntHeader}, or {@link #addIntHeader}, respectively.
     * @param name the name of the response header whose value to return
     * @return the value of the response header with the given name,
     * or <tt>null</tt> if no header with the given name has been set on this response
     * @since Servlet 3.0
     */
     ...
     * The trailer header field value is defined as a comma-separated list (see Section 3.2.2 and Section 4.1.2 of RFC 7230).</p>
     *
     * The supplier will be called within the scope of whatever thread/call causes the response content to be completed.
     * Typically this will be any thread calling close() on the output stream or writer.</p>
     *
     * The trailers that run afoul of the provisions of section 4.1.2 of  RFC 7230 are ignored.</p>
     *
     * The RFC requires the name of every key that is to be in the
     * supplied Map is included in the comma separated list that is the value of the "Trailer" response header.
	 * The application is responsible for ensuring this requirement is met.
	 * Failure to do so may lead to  interoperability failures.</p>
     *
     * @implSpec The default implementation is a no-op.
     * @param supplier the supplier of trailer headers
     * @exception IllegalStateException if it is invoked after the response has has been committed,
     *         or the trailer is not supported in the request, for instance,
     *         the underlying protocol is HTTP 1.0, or the response is not in chunked encoding in HTTP 1.1.
     * @since Servlet 4.0
     */
     ...
     * moved to another location, but that future references should still use the original URI to access the resource.
    ...
    * Request-URI for future requests.
	 * (HTTP/1.1) To represent the status code (302), it is recommended to use this variable.
    */
     ...
     * the request because the request entity is larger than the server is willing or able to process.
     */
     ...
     * invalid response from a server it consulted when acting as a  proxy or gateway.
     */
     ...
     * a timely response from the upstream server while acting as a gateway or proxy.
     */
