...
/**
 * Spring's implementation of Hibernate 5.3's {@link BeanContainer} SPI, delegating to a Spring {@link ConfigurableListableBeanFactory}.
 * xmlBeanDefinitionReaderAuto-configured by {@link LocalSessionFactoryBean#setBeanFactory},
...
public final class SpringBeanContainer implements BeanContainer {
	...
	private SpringContainedBean<?> createBean(Class<?> beanType, LifecycleOptions lifecycleOptions, BeanInstanceProducer fallbackProducer) {
		try {
	...
	private SpringContainedBean<?> createBean(String name, Class<?> beanType, LifecycleOptions lifecycleOptions, BeanInstanceProducer fallbackProducer) {
		try {
			if (lifecycleOptions.useJpaCompliantCreation()) {
