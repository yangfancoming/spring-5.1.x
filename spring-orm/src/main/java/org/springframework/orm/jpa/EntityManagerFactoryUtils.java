 ...
 *
 * xmlBeanDefinitionReaderMainly intended for internal use within the framework.
 *
...
public abstract class EntityManagerFactoryUtils {
	 ...
	 * xmlBeanDefinitionReaderThe specified unit name will be matched against the configured
	 ...
	 * xmlBeanDefinitionReaderIf no unit name has been given, this method will search for a default
	 ...
	 * xmlBeanDefinitionReaderNote: Will return {@code null} if no thread-bound EntityManager found!
	 ...
	 * Obtain a JPA EntityManager from the given factory. Is aware of a corresponding
	 * EntityManager bound to the current thread, e.g. when using JpaTransactionManager.
	 * xmlBeanDefinitionReaderSame as {@code getEntityManager}, but throwing the original PersistenceException.
	 * @param emf the EntityManagerFactory to create the EntityManager with
	 * @param properties the properties to be passed into the {@code createEntityManager}
	 * call (may be {@code null})
	 * @return the EntityManager, or {@code null} if none found
	 ...
	 * xmlBeanDefinitionReaderThis method sets the JPA 2.0 query hint "javax.persistence.query.timeout" accordingly.
	 ...
	 * xmlBeanDefinitionReaderThe most important cases like object not found or optimistic locking failure
