 ...
 *
 * xmlBeanDefinitionReaderIn general, it is recommended to derive from {@link DefaultJpaDialect} instead
...
public interface JpaDialect extends PersistenceExceptionTranslator {
	 ...
	 * xmlBeanDefinitionReaderAn implementation can configure the JPA Transaction object and then
	 * invoke {@code begin}, or invoke a special begin method that takes,
	 * for example, an isolation level.
	 * xmlBeanDefinitionReaderAn implementation can apply the read-only flag as flush mode. In that case,
	 ...
	 * xmlBeanDefinitionReaderImplementations can also use the Spring transaction name, as exposed by the
	 ...
	 * xmlBeanDefinitionReaderThis method also allows for exposing savepoint capabilities if supported by
	 ...
	 * a transaction data object can be returned that holds the previous flush mode
	 * (and possibly other data), to be reset in {@code cleanupTransaction}.
	 * xmlBeanDefinitionReaderImplementations can also use the Spring transaction name to optimize for
	 ...
	 * xmlBeanDefinitionReaderAn implementation can, for example, reset read-only flag and
	 ...
	 * xmlBeanDefinitionReaderThis strategy is necessary as JPA does not provide a standard way to retrieve
	 * the underlying JDBC Connection (due to the fact that a JPA implementation might not
	 * work with a relational database at all).
	 * xmlBeanDefinitionReaderImplementations are encouraged to return an unwrapped Connection object, i.e.
	 ...
	 * xmlBeanDefinitionReaderIn a simple case where the returned Connection will be auto-closed with the
	 ...
	 * xmlBeanDefinitionReaderAn implementation might simply do nothing, if the Connection returned
