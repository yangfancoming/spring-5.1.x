 ...
 *
 * xmlBeanDefinitionReaderEncapsulates the common functionality between the different JPA bootstrap
 * contracts (standalone as well as container).
 *
 * xmlBeanDefinitionReaderImplements support for standard JPA configuration conventions as well as
 * Spring's customizable {@link JpaVendorAdapter} mechanism, and controls the
 * EntityManagerFactory's lifecycle.
 *
 * xmlBeanDefinitionReaderThis class also implements the
...
public abstract class AbstractEntityManagerFactoryBean implements
	 ...
	 * xmlBeanDefinitionReaderDefault is none, indicating the default EntityManagerFactory
	 ...
	 * xmlBeanDefinitionReaderCan be populated with a String "value" (parsed via PropertiesEditor) or a
	 ...
	 * xmlBeanDefinitionReaderThe default will be taken from the specific JpaVendorAdapter, if any,
	 ...
	 * xmlBeanDefinitionReader{@code EntityManagerFactory} initialization will then switch into background
	 ...
	 * xmlBeanDefinitionReaderUses the dialect's conversion if possible; otherwise falls back to
