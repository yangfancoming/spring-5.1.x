 ...
 * directly under the {@code <beans>} tags and {@link #decorate} when it encounters a custom tag directly under a {@code <bean>} tag.
 *
 * Developers writing their own custom element extensions typically will not implement this interface directly,but rather make use of the provided {@link NamespaceHandlerSupport} class.
 * @since 2.0
...
public interface NamespaceHandler {
	 ...
	 * {@link org.springframework.beans.factory.support.BeanDefinitionRegistry} that is embedded in the supplied {@link ParserContext}.
	 * Implementations should return the primary {@code BeanDefinition}
	 * that results from the parse phase if they wish to be used nested inside (for example) a {@code <property>} tag.
	 ...
	 * {@link Element}, depending on whether a custom attribute or element is being parsed.
	 * Implementations may choose to return a completely new definition, which will replace the original definition in the resulting
	 * {@link org.springframework.beans.factory.BeanFactory}.
	 * The supplied {@link ParserContext} can be used to register any additional beans needed to support the main definition.
	 ...
	 * A {@code null} value is strictly speaking invalid, but will be leniently treated like the case where the original bean definition gets returned.
	 */
	@Nullable
