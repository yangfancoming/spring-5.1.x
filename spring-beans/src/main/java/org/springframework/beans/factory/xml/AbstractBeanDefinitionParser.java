...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 * Otherwise, the ID is extracted from the "id" attribute, potentially with a @link #shouldGenerateIdAsFallback() fallback} to a generated id.
	 ...
	 * @param parserContext the object encapsulating the current state of the parsing process; provides access to a {@link org.springframework.beans.factory.support.BeanDefinitionRegistry}
	...
	/**
	 * Register the supplied {@link BeanDefinitionHolder bean} with the supplied {@link BeanDefinitionRegistry registry}.
	 ...
	 * The default implementation registers the supplied {@link BeanDefinitionHolder bean}  with the supplied {@link BeanDefinitionRegistry registry} only if the {@code isNested}
	 ...
	 * Note that this flag is about <i>always</i> generating an ID; the parser won't even check for an "id" attribute in this case.
	 ...
	 * Disabled by default; subclasses can override this to enable ID generation as fallback:
	 * The parser will first check for an "id" attribute in this case,only falling back to a generated ID if no value was specified.
	 ...
	 * {@link org.springframework.beans.factory.parsing.BeanComponentDefinition}  event after parsing the bean definition.
	 * This implementation returns {@code true} by default; that is, an event will be fired when a bean definition has been completely parsed.
	 * Override this to return {@code false} in order to suppress the event.
	 * @return {@code true} in order to fire a component registration event after parsing the bean definition; {@code false} to suppress the event
