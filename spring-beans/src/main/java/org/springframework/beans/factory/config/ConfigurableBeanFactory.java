...
public interface ConfigurableBeanFactory extends HierarchicalBeanFactory, SingletonBeanRegistry {
	 ...
	 * @param beanClassLoader the class loader to use,or {@code null} to suggest the default class loader
	 ...
	 * A temporary ClassLoader is usually just specified if <i>load-time weaving</i> is involved, to make sure that actual bean classes are loaded as lazily as possible.
	 * The temporary loader is then removed once the BeanFactory completes its bootstrap phase.
	 * @since 2.5
	 ...
	 * Default is on. Turn this flag off to enable hot-refreshing of bean definition objects and in particular bean classes.
	 * If this flag is off, any creation of a bean instance will re-query the bean class loader for newly resolved classes.
	 ...
	 * An ApplicationContext will typically set a standard expression strategy here, supporting "#{...}" expressions in a Unified EL compatible style.
	 * @since 3.0
	 * 为bean定义值中的表达式指定解析策略。
	 ...
	 * This avoids the need for synchronization on custom editors;
	 ...
	 * Note that this method will register a shared custom editor instance; access to that instance will be synchronized for thread-safety.
	 * It is  generally preferable to use {@link #addPropertyEditorRegistrar} instead of this method, to avoid for the need for synchronization on custom editors.
	...
	/**
	 * Obtain a type converter as used by this BeanFactory. This may be a fresh  instance for each call, since TypeConverters are usually <i>not</i> thread-safe.
	...
	/**
	 * Determine whether an embedded value resolver has been registered with this bean factory, to be applied through {@link #resolveEmbeddedValue(String)}.
	 * @since 4.3
	 ...
	 * Note: Post-processors submitted here will be applied in the order of registration; any ordering semantics expressed through implementing the
	 * {@link org.springframework.core.Ordered} interface will be ignored.
	 * Note  that autodetected post-processors (e.g. as beans in an ApplicationContext)  will always be applied after programmatically registered ones.
