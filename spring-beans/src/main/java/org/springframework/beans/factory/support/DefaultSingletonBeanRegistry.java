...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * 用于存放【成熟态】的单例bean实例，从该缓存中取出的 bean 可以直接使用
	 */
	 ...
	 * 缓存beanName和bean实例 key-->beanName,value-->beanInstance
	 * 用于存放【纯净态】的早期单例bean实例（尚未填充属性），主要用于解决 一般情况下的循环依赖
	 */
	 ...
	 * 单例对象工厂的cache，存放 bean 工厂对象，主要用于解决基于AOP代理的循环依赖
	 */
	...
	*/
	 ...
	 * 例如： a dependsOn b,c   则存储为  k = "a", v = {"b","c"}
	 */
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		// 如果获取不到，且对象正在创建中，就从二级缓存池中获取。
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			synchronized (singletonObjects) {
				// 从二级缓冲池中获取提前曝光的 bean，用于处理循环引用
				singletonObject = earlySingletonObjects.get(beanName);
				// 如果二级缓冲池也没有，且允许提前曝光 bean 实例，则从三级缓冲池中获取一个原始的（raw）bean（尚未填充属性）
				...
				if (singletonObject == null && allowEarlyReference) {
					...
					if (singletonFactory != null) {
						...
						*/
						...
						// 放入二级缓存中，如果还有其他 bean 依赖当前 bean，其他 bean 可以直接从 earlySingletonObjects 取结果
						earlySingletonObjects.put(beanName, singletonObject);
						// 添加到二级缓存池后，需要从三级缓存中移除
						singletonFactories.remove(beanName);
					}
				}
			}
	 ...
	 * (例如A定义的构造函数依赖了B对象，得先去创建B对象，此时A处于创建中)
	 * @param beanName the name of the bean
	 ...
	 * @param beanName the name of the bean to check   当前要创建的bean
	 * @param dependentBeanName the name of the dependent bean 当前要创建的bean中所需要的依赖bean
	 * @since 4.0
