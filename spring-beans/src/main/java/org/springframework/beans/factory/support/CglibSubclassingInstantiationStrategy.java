 ...
 * Uses CGLIB to generate subclasses dynamically if methods need to be overridden by the container to implement <em>Method Injection</em>.
 * @since 1.1
 */
public class CglibSubclassingInstantiationStrategy extends SimpleInstantiationStrategy {
	...
	// Index in the CGLIB callback array for a method that should be overridden using generic <em>method replacer</em> functionality.
	...
	private static class CglibSubclassCreator {
		...
		public Object instantiate(@Nullable Constructor<?> ctor, Object... args) {
			Class<?> subclass = createEnhancedSubclass(beanDefinition);
			Object instance;
			if (ctor == null) {
				...
				try {
					...
					throw new BeanInstantiationException(beanDefinition.getBeanClass(),"Failed to invoke constructor for CGLIB enhanced subclass [" + subclass.getName() + "]", ex);
				}
			}
		...
		private Class<?> createEnhancedSubclass(RootBeanDefinition beanDefinition) {
			...
			if (owner instanceof ConfigurableBeanFactory) {
				ClassLoader cl = ((ConfigurableBeanFactory) owner).getBeanClassLoader();
	...
	private static class CglibIdentitySupport {
		...
		public boolean equals(Object other) {
			return (other != null && getClass() == other.getClass() && beanDefinition.equals(((CglibIdentitySupport) other).beanDefinition));
		}
	 ...
	 * as thread context ClassLoader for the time of class generation (in order for ASM to pick it up when doing common superclass resolution).
	 */
	private static class ClassLoaderAwareGeneratorStrategy extends DefaultGeneratorStrategy {
		...
		public byte[] generate(ClassGenerator cg) throws Exception {
			...
			boolean overrideClassLoader = !classLoader.equals(threadContextClassLoader);
	...
	private static class LookupOverrideMethodInterceptor extends CglibIdentitySupport implements MethodInterceptor {
		...
		public Object intercept(Object obj, Method method, Object[] args, MethodProxy mp) throws Throwable {
			...
			if (StringUtils.hasText(lo.getBeanName())) {
				return (argsToUse != null ? owner.getBean(lo.getBeanName(), argsToUse) : owner.getBean(lo.getBeanName()));
			}else {
				return (argsToUse != null ? owner.getBean(method.getReturnType(), argsToUse) : owner.getBean(method.getReturnType()));
			}
		}
	}
