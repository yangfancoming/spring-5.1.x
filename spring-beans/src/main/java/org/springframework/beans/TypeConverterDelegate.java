				...
				if (String.class == requiredType && ClassUtils.isPrimitiveOrWrapper(convertedValue.getClass())) {
					...
					if (conversionAttemptEx == null && !requiredType.isInterface() && !requiredType.isEnum()) {
						try {
							...
							if (logger.isTraceEnabled()) logger.trace("No String constructor found on type [" + requiredType.getName() + "]", ex);
						}catch (Exception ex) {
							if (logger.isDebugEnabled()) logger.debug("Construction via String failed for type [" + requiredType.getName() + "]", ex);
						}
					}
		...
		if (Enum.class == requiredType && this.targetObject != null) {
			...
			if (index > - 1) {
				...
				try {
					...
					if (logger.isTraceEnabled()) logger.trace("Enum class [" + enumType + "] cannot be loaded", ex);
				}catch (Throwable ex) {
					if (logger.isTraceEnabled()) logger.trace("Field [" + fieldName + "] isn't an enum value for type [" + enumType + "]", ex);
				}
			}
		}
	...
	private Object doConvertValue(@Nullable Object oldValue, @Nullable Object newValue,
		...
		if (editor != null && !(convertedValue instanceof String)) {
			...
			try {
				...
				if (logger.isDebugEnabled()) logger.debug("PropertyEditor [" + editor.getClass().getName() + "] does not support setValue call", ex);
				// Swallow and proceed.
			}
		}
		...
		if (requiredType != null && !requiredType.isArray() && convertedValue instanceof String[]) {
			...
			if (logger.isTraceEnabled()) logger.trace("Converting String array to comma-delimited String [" + convertedValue + "]");
		...
		if (convertedValue instanceof String) {
			if (editor != null) {
				...
				if (logger.isTraceEnabled()) logger.trace("Converting String to [" + requiredType + "] using property editor [" + editor + "]");
