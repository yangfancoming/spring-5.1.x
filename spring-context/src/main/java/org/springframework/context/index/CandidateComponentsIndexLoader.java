...
public final class CandidateComponentsIndexLoader {
	...
	/**
	 * System property that instructs Spring to ignore the index, i.e.to always return {@code null} from {@link #loadIndex(ClassLoader)}.
	 * The default is "false", allowing for regular use of the index. Switching this flag to {@code true} fulfills a corner case scenario when an index is partially
	 * available for some libraries (or use cases) but couldn't be built for the whole application. In this case, the application context fallbacks to a regular
	...
	private static CandidateComponentsIndex doLoadIndex(ClassLoader classLoader) {
		//是否需要忽略index
		if (shouldIgnoreIndex) return null;
		try {
			...
			if (logger.isDebugEnabled()) logger.debug("Loaded " + result.size() + "] index(es)");
